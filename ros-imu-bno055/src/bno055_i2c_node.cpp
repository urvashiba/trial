...
BNO055I2CNode::BNO055I2CNode(int argc, char* argv[]) {
    ...
    pub_data = nh->advertise<sensor_msgs::Imu>("data", 0);
    pub_raw = nh->advertise<sensor_msgs::Imu>("raw", 0);
    pub_mag = nh->advertise<sensor_msgs::MagneticField>("mag", 0);
    pub_temp = nh->advertise<sensor_msgs::Temperature>("temp", 0);
    pub_status = nh->advertise<diagnostic_msgs::DiagnosticStatus>("status", 0);
